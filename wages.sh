...
#and will find the difference between the average wages of both groups. Finally, this difference will be printed

#usage: variable=$(cut -d delimiter -f columns_to_keep filename | sed -n '/line_starting_with_number_to_keep/p' | 
#cut -d delimiter -f column_to_keep |awk '{variable_name+=sprintf("%number_type", $column_number)} END{printf "%number_type\newline", variable/NR}') 
val16=$(cut -d "," -f 3,4 $1 | sed -n '/^16/p'| sed 's/,/ /g'| cut -d " " -f 2| awk '{sum+=sprintf("%f",$1)}END{printf "%.6f\n",sum/NR}')
val12=$(cut -d "," -f 3,4 $1 | sed -n '/^12/p'| sed 's/,/ /g'| cut -d " " -f 2| awk '{sum+=sprintf("%f",$1)}END{printf "%.6f\n",sum/NR}')
